...
const Followers = () => {
  ...
  useEffect(() => {
    const userList = []
    const promises = []
    const unsubscribe = db.doc(`/users/${userId}`).collection("followers").onSnapshot(queryFollowingSnapshot => {
  ...
  const renderUserCard = () => {
    return followersList.map((user, index) => {
      return (
        ...
        <Grid container style={{ paddingTop: '30px' }}>
          ...
          <Grid container item xs={12}>
            ...
            <Grid container item xs={6} style={{ justifyContent: 'center' }}>
              <Typography variant='h4' style={{ textAlign: 'center', fontSize: '25px' }}>Followers</Typography>
            </Grid>
            <Grid item xs={3} />
          </Grid>
